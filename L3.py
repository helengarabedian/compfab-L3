...
def z_key(z):
    if float(layer_height) == 0.0:
        return float(z)
    index = int(round(float(z)/ float(layer_height)))
    return index * float(layer_height)
...
def gcode_move(current_pos, next_pos, feed_rate=None, should_extrude=False):
    ...
    if(should_extrude == True):
        Ldist = math.sqrt(((next_pos[0]-current_pos[0])**2)+ ((next_pos[1]-current_pos[1])**2)+ ((next_pos[2]-current_pos[2])**2))
        v_out = Ldist * ((layer_height * (extrusion_width - layer_height)) + ((math.pi * (layer_height/2)**2)))
        eVal = float_to_string(v_out / (math.pi*(filament_diameter/2.0)**2))
        move_command_str += " " + PARAM_E + eVal
...
def generate_gcode():
    ...
    for (_layerZ, line) in ordered:
        ...
        if(not are_floats_equal(line_start_position[2], current_position[2])):
            next_line = [current_position[0], current_position[1], line_start_position[2]]
            zMove = gcode_move(current_position, next_line, feed_rate = layer_change_feed_rate)
            all_move_commands.append(zMove)
